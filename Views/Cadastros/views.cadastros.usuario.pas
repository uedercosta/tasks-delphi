...
begin
  ...
  edLogin.text:= ConvertToModel(aValue).Login;
  edNome.text:= ConvertToModel(aValue).Nome;
  edSenha.text:= ConvertToModel(aValue).Senha;
  if ConvertToModel(aValue).Ativo = 0 then checkAtivo.Checked := false else checkAtivo.Checked:=true;
  edCriacao.Date:= ConvertToModel(aValue).DataCriacao;
  lblCodigo.Caption := ConvertToModel(aValue).Id.ToString;
end;
